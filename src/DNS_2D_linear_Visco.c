...
// Last modified: Tue  2 Aug 12:55:50 2016
...
void output_macro_state(complex_d *psi, complex_d *cij, double phase, double time,
	FILE *traceKE, FILE *tracePSI, lin_flow_scratch scr, flow_params params);

void setup_scratch_space(lin_flow_scratch *scr, flow_params params);

void debug_output_halfstep_variables(complex_d *psiNL, complex_d *cijNL, lin_flow_scratch scr, flow_params params);

void debug_output_fullstep_variables(complex_d *psi, complex_d *cij, lin_flow_scratch scr, flow_params params);
...
int main(int argc, char **argv) 
{
    ...
    // dynamically allocate the fields and operators
    ...
    // recalculate the base flow 
    calc_base_cij(cij, initTime, scr, params);
    calc_base_sf(psi, initTime, scr, params);
    ...
    output_macro_state(psi, cij, phase, time, tracefp, tracePSI, scr, params);
...
void setup_scratch_space(lin_flow_scratch *scr, flow_params params) 
{
    ...
    scr->act_phys_plan = fftw_plan_dft_1d(2*Mf-2,  scr->scratchin, scr->scratchout,
             FFTW_BACKWARD, fftwFlag);
    scr->act_spec_plan = fftw_plan_dft_1d(2*Mf-2,  scr->scratchin, scr->scratchout,
...
void debug_output_halfstep_variables(complex_d *psiNL, complex_d *cijNL, lin_flow_scratch scr, flow_params params)
{
    ...
    // base flow has stepped onto the half step
    ...
    // velocities and other linear terms have been calculated at the old
    // step during the streamfunction calculation
    ...
    // The linear stress terms have also been calculated at the half step during
    // the streamfunction calculation
    ...
    // The nonlinear stress terms have been calculated at the old step during
    // the stress step calculation
...
void debug_output_fullstep_variables(complex_d *psi, complex_d *cij, lin_flow_scratch scr, flow_params params)
{
    ...
    // velocities and stresses at the new time have been calculated
    ...
    // New linear stress terms have been calculated in the streamfunction
    // calculation
    ...
    // nonlinear velocity terms on the half step have been calculated during
    // the streamfunction calculation
    save_hdf5_arr("./output/udxlplpsiStar.h5", &scr.udxlplpsi[0], M);
    save_hdf5_arr("./output/vdylplpsiStar.h5", &scr.vdylplpsi[0], M);

    // nonlinear stress terms on the half step have been calculated during the
    // stress calculation
